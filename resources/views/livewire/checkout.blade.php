<div class="py-20">
    <div class="container mx-auto px-6 max-w-6xl">
        ...
        <div class="mb-16 border-l-2 border-white pl-6">
            <h1 class="text-4xl font-bold uppercase tracking-tighter text-white">Finalize_Order</h1>
            <p class="mono text-[10px] text-white/30 uppercase tracking-[0.3em] mt-2">Section: Secure_Checkout // Node: {{ request()->ip() }}</p>
        </div>
        ...
        @if ($errors->any())
            <div class="mb-12 border border-red-500/50 bg-red-500/5 p-6 relative overflow-hidden animate-pulse">
                ...
                <div class="flex items-center mb-4">
                    <span class="text-red-500 font-bold mr-3 text-xs">[!] STATUS: CRITICAL_ERROR</span>
                ...
                <form wire:submit.prevent="placeOrder" class="space-y-12">
                    <section>
                        <h2 class="text-xs font-bold uppercase tracking-widest text-white/50 mb-8 flex items-center">
                            <span class="w-8 h-[1px] bg-white/20 mr-4"></span> 01. Shipping_Protocol
                        </h2>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {{-- Имя --}}
                            <div class="space-y-2">
                                <label class="mono text-[9px] uppercase text-white/40 ml-1">Full_Name</label>
                                <input type="text" wire:model.blur="name" placeholder="Example: Major Motoko Kusanagi"
                            ...
                            {{-- Email --}}
                            <div class="space-y-2">
                                <label class="mono text-[9px] uppercase text-white/40 ml-1">Contact_Email</label>
                                <input type="email" wire:model.blur="email" placeholder="Example: sector7@nexus.io"
                            ...
                            <div class="space-y-2 md:col-span-2">
                                <label class="mono text-[9px] uppercase text-white/40 ml-1">Secure_Line (Phone)</label>
                                <input type="text" wire:model.blur="phone" placeholder="Example: +1 000 000 0000"
                            ...
                            {{-- Адрес --}}
                            <div class="space-y-2 md:col-span-2">
                                <label class="mono text-[9px] uppercase text-white/40 ml-1">Drop_Coordinates (Address)</label>
                                <textarea wire:model.blur="address" rows="3" placeholder="Example: Night City, Sector 04, Apartment 42"
                    ...
                    <button type="submit" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="placeOrder">Execute_Order_v1.0</span>
            ...
            <div class="lg:col-span-5">
                <div class="bg-white/[0.02] border border-white/5 p-8 sticky top-8">
                    <h2 class="mono text-[10px] uppercase text-white/40 tracking-widest mb-8 flex items-center">
                        <span class="w-2 h-2 bg-white/20 mr-3"></span> Order_Summary
                    </h2>
                        ...
                        @foreach($cart as $item)
                            <div class="flex justify-between items-start group">
                                <div>
                                    ...
                                    <p class="mono text-[9px] text-white/30 uppercase mt-1">Units: {{ $item['quantity'] }} // PPU: ${{ number_format($item['price'], 0) }}</p>
                                </div>
                    ...
                    <div class="border-t border-white/10 pt-6 space-y-4">
                        ...
                        <div class="flex justify-between mono text-[10px] uppercase">
                            <span class="text-white/40">Shipping_Fee</span>
                            <span class="text-green-500">Free_Protocol</span>
                        </div>
                        <div class="flex justify-between items-end pt-6 border-t border-white/5">
                            <span class="mono text-[10px] uppercase text-white/40">Total_Amount</span>
