    ...
    <header class="container mx-auto px-6 py-12 lg:py-20 border-b border-white/5">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
            ...
            <div class="text-left md:text-right space-y-2">
                <p class="text-[9px] text-white/40 tracking-[0.2em] uppercase">{{ __('ui.labs.access') }}</p>
                <p class="text-[9px] text-white/20 tracking-[0.2em] uppercase">{{ __('ui.labs.local_time') }}: {{ now(config('app.timezone'))->format('H:i:s') }}</p>
            </div>
        </div>
    </header>
    ...
    <main class="container mx-auto px-6 py-8 lg:py-12">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
            ...
            <div class="lg:col-span-4 space-y-8">
                <div>
                    <h2 class="text-[10px] text-white/20 uppercase tracking-[0.4em] mb-6">{{ __('ui.labs.select') }}</h2>
                ...
                <div class="hidden lg:block p-6 border border-dashed border-white/5 opacity-40 space-y-3">
                    <p class="text-[7px] uppercase tracking-[0.3em]">> {{ __('ui.labs.loading_1') }}</p>
                    <p class="text-[7px] uppercase tracking-[0.3em]">> {{ __('ui.labs.loading_2') }}</p>
            ...
            <div class="lg:col-span-8">
                <div class="space-y-12 lg:space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-700" wire:key="{{ $activeProject['id'] }}">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start border-b border-white/10 pb-8 gap-6">
                        <div class="space-y-4">
                            ...
                            <div class="flex flex-wrap gap-4 text-[9px] tracking-[0.2em] text-white/40 uppercase font-bold">
                                <span class="text-white border border-white/20 px-2 py-1">{{ __('ui.labs.status') }}: {{ $activeProject['status'] }}</span>
                                <span class="py-1">{{ __('ui.labs.year') }}: {{ $activeProject['year'] }}</span>
                                <span class="py-1">{{ __('ui.labs.class') }}</span>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                        <div class="space-y-4">
                            <h4 class="text-[10px] text-white/40 uppercase tracking-[0.3em] font-bold">{{ __('ui.labs.overview') }}</h4>
                            <p class="text-[12px] text-zinc-400 leading-loose tracking-[0.1em] uppercase">
                                {{ $activeProject['description'] }}
                            </p>
                        </div>
                        <div class="space-y-4">
                            <h4 class="text-[10px] text-white/40 uppercase tracking-[0.3em] font-bold">{{ __('ui.labs.materials') }}</h4>
                    ...
                    <div class="space-y-6">
                        <h4 class="text-[10px] text-white/40 uppercase tracking-[0.3em] font-bold">{{ __('ui.labs.specs') }}</h4>
                    ...
                    <div class="space-y-8">
                        <h4 class="text-[10px] text-white/40 uppercase tracking-[0.3em] font-bold">{{ __('ui.labs.log') }}</h4>
                        <div class="space-y-0">
