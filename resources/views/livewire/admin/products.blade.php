...
<div class="mt-4">
    ...
    <div class="flex items-center space-x-4">
        <input type="file" wire:model="image" class="text-xs text-zinc-500 file:mr-4 file:py-2 file:px-4 file:border-0 file:bg-zinc-800 file:text-zinc-300 hover:file:bg-zinc-700">
        
        <div wire:loading wire:target="image" class="text-[10px] text-green-500 animate-pulse uppercase">Uploading...</div>
    </div>

    <div class="mt-4">
        @if ($image)
            <img src="{{ $image->temporaryUrl() }}" class="w-24 h-24 object-cover border border-zinc-700">
        @elseif ($existingImage)
            <img src="{{ asset('storage/' . $existingImage) }}" class="w-24 h-24 object-cover border border-zinc-700">
        @endif
    </div>
</div>
